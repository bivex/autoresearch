...
def main():
    if not os.path.exists(DATA_FILE):
        print("Error: Data file not found. Run prepare.py first.")
        return
    ...
    while time.time() - t_start < TIME_BUDGET:
        ...
        print(f"Iter {iter_idx:3d} | Train LL: {total_train_ll.item():.1f} | Time: {elapsed:.1f}s")
    ...
    val_bpn = -total_val_ll.item() / (math.log(2) * total_notes)
    print(f"\nFinal Validation LL: {total_val_ll.item():.1f}")
    print(f"Final val_bpb: {val_bpn:.6f}")
